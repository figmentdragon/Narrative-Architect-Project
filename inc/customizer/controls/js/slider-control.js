wp.customize.controlConstructor['creativityarchitect-range-slider'] = wp.customize.Control.extend({
	...
	ready: function() {
		...
		var control = this,
		    ...
		    inputDefault,
		    changeAction,
			controlClass = '.customize-control-creativityarchitect-range-slider',
			footerActions = jQuery( '#customize-footer-actions' );
		
		// Set up the sliders
		jQuery( '.creativityarchitect-slider' ).each( function() {
		...
		jQuery( controlClass + ' .creativityarchitect_range_value input[type=number]' ).on( 'input', function() {
			value = jQuery( this ).attr( 'value' );
			if ( '' == value ) {
				value = -1;
			}
			jQuery( this ).closest( 'label' ).find( '.creativityarchitect-slider' ).slider( 'value', parseFloat(value)).change();
		});

		// Handle the reset button
		jQuery( controlClass + ' .creativityarchitect-reset' ).on( 'click', function() {
			var icon = jQuery( this ),
				visible_area = icon.closest( '.creativityarchitect-range-title-area' ).next( '.creativityarchitect-range-slider-areas' ).children( 'label:visible' ),
				input = visible_area.find( 'input[type=number]' ),
				slider_value = visible_area.find( '.creativityarchitect-slider' ),
				visual_value = visible_area.find( '.creativityarchitect_range_value' ),
		...
		jQuery( controlClass + ' .creativityarchitect-range-slider-control' ).each( function() {
			var _this = jQuery( this );
			_this.find( '.creativityarchitect-device-controls' ).children( 'span:first-child' ).addClass( 'selected' );
			_this.find( '.range-option-area:first-child' ).show();
		});
		
		// Do stuff when device icons are clicked
		jQuery( controlClass + ' .creativityarchitect-device-controls > span' ).on( 'click', function( event ) {
			var device = jQuery( this ).data( 'option' );
			
			jQuery( controlClass + ' .creativityarchitect-device-controls span' ).each( function() {
				var _this = jQuery( this );
			...
			jQuery( controlClass + ' .creativityarchitect-range-slider-areas label' ).each( function() {
				var _this = jQuery( this );
				if ( device == _this.attr( 'data-option' ) ) {
					_this.show();
					_this.siblings().hide();
