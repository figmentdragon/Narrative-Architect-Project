( function( api ) {
	...
	api.controlConstructor['creativityarchitect-customizer-typography'] = api.Control.extend( {
		ready: function() {
			var control = this;

			control.container.on( 'change', '.creativityarchitect-font-family select',
				function() {
					...
					setTimeout( function() {
						// Send our request to the creativityarchitect_get_all_google_fonts_ajax function
						var response = jQuery.getJSON({
							type: 'POST',
							url: ajaxurl,
							data: {
								action: 'creativityarchitect_get_all_google_fonts_ajax',
								creativityarchitect_customize_nonce: creativityarchitect_customize.nonce
							},
							async: false,
							dataType: 'json',
						});
						...
						if ( id in fonts ) {
							...
							jQuery( '.creativityarchitect-font-family select' ).not( _this ).each( function( key, select ) {
								var parent = jQuery( this ).closest( '.creativityarchitect-font-family' );

								if ( _value == jQuery( select ).val() && _this.data( 'category' ) !== jQuery( select ).data( 'category' ) ) {
									if ( ! got_variants ) {
										updated_variants = jQuery( parent.next( '.creativityarchitect-font-variant' ).find( 'select' ) ).val();
										got_variants = true;
									}
								}
							} );
							...
							_this.closest( '.creativityarchitect-font-family' ).next( 'div' ).show();
							...
							_this.closest( '.creativityarchitect-font-family' ).next( 'div' ).hide();
			...
			control.container.on( 'change', '.creativityarchitect-font-variant select',
				function() {
					var _this = jQuery( this );
					...
					jQuery( '.creativityarchitect-font-variant select' ).each( function( key, value ) {
						var this_control = jQuery( this ).closest( 'li' ).attr( 'id' ).replace( 'customize-control-', '' );
						var parent = jQuery( this ).closest( '.creativityarchitect-font-variant' );
			...
			control.container.on( 'change', '.creativityarchitect-font-category input',
				function() {
					control.settings['category'].set( jQuery( this ).val() );
				}
			);

			control.container.on( 'change', '.creativityarchitect-font-weight select',
				function() {
					control.settings['weight'].set( jQuery( this ).val() );
				}
			);

			control.container.on( 'change', '.creativityarchitect-font-transform select',
				function() {
...
jQuery( document ).ready( function($) {
	...
	jQuery( '.creativityarchitect-font-family select' ).selectWoo();
	jQuery( '.creativityarchitect-font-variant' ).each( function( key, value ) {
		var _this = $( this );
	...
	$( ".creativityarchitect-font-family" ).each( function( key, value ) {
		var _this = $( this );
		if ( $.inArray( _this.find( 'select' ).val(), typography_defaults ) !== -1 ) {
			_this.next( '.creativityarchitect-font-variant' ).hide();
		}
	});
