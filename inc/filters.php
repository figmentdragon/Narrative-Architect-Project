...
add_filter('excerpt_length', 'creativityarchitect_excerpt_length');
...
if (!function_exists('creativityarchitect_the_attached_image')) :
    ...
    function creativityarchitect_the_attached_image() {
         ...
         * }
         */
        $attachment_size = apply_filters('creativityarchitect_attachment_size', array(960, 640));
 ...
 */
function creativityarchitect_wp_title($title, $sep) {
    global $paged, $page;
    if (is_feed()) {
        return $title;
    }
    ...
    if ($paged >= 2 || $page >= 2) {
        $title = "$title $sep " . sprintf(__('Page %s', 'creativityarchitect'), max($paged, $page));
    }
    return $title;
}

add_filter('wp_title', 'creativityarchitect_wp_title', 10, 2);
...
function creativityarchitect_anchor_content_h2($content) {
...
add_filter('the_content', 'creativityarchitect_anchor_content_h2');
 ...
 */
function creativityarchitect_replace_ptags_around_images_with_figure($content) {
...
add_filter('the_content', 'creativityarchitect_replace_ptags_around_images_with_figure');
 ...
 */
function creativityarchitect_remove_ptags_around_embeds($content) {
...
add_filter('the_content', 'creativityarchitect_remove_ptags_around_embeds');
 ...
 */
function creativityarchitect_img_caption_shortcode_filter($val, $attr, $content = null) {
...
add_filter('img_caption_shortcode', 'creativityarchitect_img_caption_shortcode_filter', 10, 3);
...
function creativityarchitect_remove_comment_allowed_tags_notes($defaults) {
    $defaults['comment_notes_after'] = '';
    return $defaults;
}

add_filter('comment_form_defaults', 'creativityarchitect_remove_comment_allowed_tags_notes');

/**
 * Remove 'hentry' from post_class()
 * @link https://gist.github.com/jaredatch/1629862
 */
function creativityarchitect_remove_hentry_from_homepage($class) {
    if(is_front_page()){
...
add_filter('post_class', 'creativityarchitect_remove_hentry_from_homepage');
