...
class Theme_Options {
	...
	function __construct() {
        ...
        add_action( 'admin_enqueue_scripts', 'enqueue_options_scripts' );
	...
	public function display_import_export_settings() {
		...
		wp_nonce_field( 'settings_import_nonce', 'settings_import_nonce' );
	...
	public function display_setting( $args = array() ) {
		...
		switch ( $type ) {
			...
			case 'checkbox':
				...
				echo '<input class="checkbox' . $field_class . '" type="checkbox" id="' . $id . '" name="options[' . $id . ']" value="1" ' . checked( $options[$id], 1, false ) . ' /> <label for="' . $id . '">' . $desc . '</label>';

				break;

			case 'select':
				echo '<select class="select' . $field_class . '" name="options[' . $id . ']">';
			...
			case 'radio':
				$i = 0;
				foreach ( $choices as $value => $label ) {
					echo '<input class="radio' . $field_class . '" type="radio" name="options[' . $id . ']" id="' . $id . $i . '" value="' . esc_attr( $value ) . '" ' . checked( $options[$id], $value, false ) . '> <label for="' . $id . $i . '">' . $label . '</label>';
			...
			case 'textarea':
				echo '<textarea class="' . $field_class . '" id="' . $id . '" name="options[' . $id . ']" placeholder="' . $std . '" rows="10" cols="80">' . format_for_editor( $options[$id] ) . '</textarea>';
			...
			case 'richtextarea':
				...
				$rich_settings = array( 'textarea_name' => 'options[' . $id . ']' , 'editor_height' => '200',  'tabindex'  => "3", 'editor_class' => $field_class );
			...
			case 'medialoader':
				...
				if ( $options[$id] )  {
					...
					echo '<img id="previewimage_' . $id . '" src="' . get_stylesheet_directory() . '/images/default-header-640.jpg" alt="default header image" />';
				}

				echo '<input type="hidden" name="options[' . $id . ']" id="' . $id . '" value="' . $options[$id]  . '" />
  <br /><input type="button" class="upload_image_button button-primary" name="_button' . $id .'" id="_button' . $id .'" data-options_id="' . $id  . '" data-uploader_title="Set Default Header Image" data-uploader_button_text="Select Image" value="Set/Change Image" />
</div><!-- uploader -->';
			...
			case 'password':
				echo '<input class="regular-text' . $field_class . '" type="text" id="' . $id . '" name="options[' . $id . ']" value="' . esc_attr( $options[$id] ) . '" /> <input type="button" id="showHide" value="Show" /> ';
			...
			case 'text':
			default:
				echo '<input class="regular-text' . $field_class . '" type="text" id="' . $id . '" name="options[' . $id . ']" placeholder="' . $std . '" value="' . esc_attr( $options[$id] ) . '" />';
	...
	public function export_settings() {
		...
		if ( ! wp_verify_nonce( $_POST['settings_export_nonce'], 'settings_export_nonce' ) )
			return;
		...
		header( 'Content-Disposition: attachment; filename=settings-export-' . date( 'm-d-Y' ) . '.json' );
		header( "Expires: 0" );
	...
	public function import_settings() {
		...
		if ( ! wp_verify_nonce( $_POST['settings_import_nonce'], 'settings_import_nonce' ) )
			return;
		...
		$curr_options = get_option( 'options' );
		...
		// return to options
		wp_safe_redirect( admin_url( 'https://splot.test/wp-admin/themes.php?page=options&settings-imported=1' ) ); exit;
	...
	public function initialize_settings() {
		...
		update_option( 'options', $default_settings );
	...
	public function register_settings() {
		...
		register_setting( 'options', 'options', array ( &$this, 'validate_settings' ) );

		foreach ( $this->sections as $slug => $title ) {
			add_settings_section( $slug, $title, array( &$this, $this->section_callbacks[$slug] ), 'options' );
		}
	...
	public function create_setting( $args = array() ) {
		...
		add_settings_field( $id, $title, array( $this, 'display_setting' ), 'options', $section, $field_args );
