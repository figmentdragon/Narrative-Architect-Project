...
if ( ! function_exists( 'creativityarchitect_enqueue_google_fonts' ) ) {
	add_action( 'wp_enqueue_scripts', 'creativityarchitect_enqueue_google_fonts', 0 );
	/**
	 * Add Google Fonts to wp_head if needed.
	 *
	 */
	function creativityarchitect_enqueue_google_fonts() {
		...
		$not_google = str_replace( ' ', '+', creativityarchitect_typography_default_fonts() );
		...
		if ( ! empty( $font_settings ) ) {
			...
			foreach ( $font_settings as $key ) {
				...
				if ( ! isset( $creativityarchitect_settings[$key] ) ) {
					continue;
				}

				// If our value is still using the old format, fix it
				if ( strpos( $creativityarchitect_settings[$key], ':' ) !== false ) {
					$creativityarchitect_settings[$key] = current( explode( ':', $creativityarchitect_settings[$key] ) );
				}

				// Replace the spaces in the names with a plus
				$value = str_replace( ' ', '+', $creativityarchitect_settings[$key] );

				// Grab the variants using the plain name
				$variants = creativityarchitect_get_google_font_variants( $creativityarchitect_settings[$key], $key );
		...
		$google_fonts = apply_filters( 'creativityarchitect_typography_google_fonts', $google_fonts );

		// Get the subset
		$subset = apply_filters( 'creativityarchitect_fonts_subset','' );
		...
		$default_fonts = creativityarchitect_DEFAULT_FONTS;

		// Create our URL using the arguments
		$fonts_url = add_query_arg( $font_args, '//fonts.googleapis.com/css' );

		// Enqueue our fonts
		wp_enqueue_style('creativityarchitect-default-fonts', $default_fonts, array(), null, 'all' );
		if ( $google_fonts ) {
			wp_enqueue_style('creativityarchitect-fonts', $fonts_url, array(), null, 'all' );
		}
	}
}

if ( ! function_exists( 'creativityarchitect_default_fonts_customize_register' ) ) {
	add_action( 'customize_register', 'creativityarchitect_default_fonts_customize_register' );
	/**
	 * Build our Typography options
	 *
	 */
	function creativityarchitect_default_fonts_customize_register( $wp_customize ) {
		if ( function_exists( 'creativityarchitect_fonts_customize_register' ) ) {
			// Bail if creativityarchitect Premium is active
			return;
		}

		$defaults = creativityarchitect_get_default_fonts();

		if ( method_exists( $wp_customize,'register_control_type' ) ) {
			$wp_customize->register_control_type( 'creativityarchitect_Typography_Customize_Control' );
		...
		$wp_customize->add_section(
			'font_section',
			array(
				'title' => __( 'Typography', 'creativityarchitect' ),
		...
		$wp_customize->add_setting(
			'creativityarchitect_settings[font_body]',
			array(
		...
		$wp_customize->add_setting(
			...
			array(
				...
				'sanitize_callback' => 'creativityarchitect_sanitize_variants'
			)
		);

		$wp_customize->add_setting(
			'creativityarchitect_settings[body_font_weight]',
			array(
		...
		$wp_customize->add_setting(
			'creativityarchitect_settings[body_font_transform]',
			array(
		...
		$wp_customize->add_control(
			new creativityarchitect_Typography_Customize_Control(
				$wp_customize,
				'body_typography',
				array(
					'section' => 'font_section',
					'priority' => 1,
					'label' => __( 'Text Font', 'creativityarchitect' ),
					'settings' => array(
						'family' => 'creativityarchitect_settings[font_body]',
						'variant' => 'font_body_variants',
						'category' => 'font_body_category',
						'weight' => 'creativityarchitect_settings[body_font_weight]',
						'transform' => 'creativityarchitect_settings[body_font_transform]',
					),
				)
			)
		);
		...
		$wp_customize->add_setting(
			'creativityarchitect_settings[body_font_size]',
			array(
		...
		$wp_customize->add_control(
			new creativityarchitect_Range_Slider_Control(
				...
				array(
					...
					'description' => __( 'Font size', 'creativityarchitect' ),
					'section' => 'font_section',
					'settings' => array(
						'desktop' => 'creativityarchitect_settings[body_font_size]',
					),
					'choices' => array(
						'desktop' => array(
							'min' => 6,
		...
		$wp_customize->add_setting(
			'creativityarchitect_settings[body_line_height]',
			array(
				'default' => $defaults['body_line_height'],
				'type' => 'option',
				'sanitize_callback' => 'creativityarchitect_sanitize_decimal_integer',
				'transport' => 'postMessage'
			)
		);
		...
		$wp_customize->add_control(
			new creativityarchitect_Range_Slider_Control(
				...
				array(
					'type' => 'creativityarchitect-range-slider',
					'description' => __( 'Line height', 'creativityarchitect' ),
					'section' => 'font_section',
					'settings' => array(
						'desktop' => 'creativityarchitect_settings[body_line_height]',
					),
					'choices' => array(
						'desktop' => array(
							'min' => 1,
		...
		$wp_customize->add_setting(
			'creativityarchitect_settings[paragraph_margin]',
			array(
		...
		$wp_customize->add_control(
			new creativityarchitect_Range_Slider_Control(
				...
				array(
					'type' => 'creativityarchitect-range-slider',
					'description' => __( 'Paragraph margin', 'creativityarchitect' ),
					'section' => 'font_section',
					'settings' => array(
						'desktop' => 'creativityarchitect_settings[paragraph_margin]',
					),
					'choices' => array(
						'desktop' => array(
							'min' => 0,
...
if ( ! function_exists( 'creativityarchitect_get_all_google_fonts' ) ) {
	/**
	 ...
	 */
	function creativityarchitect_get_all_google_fonts( $amount = 'all' ) {
		...
		if ( apply_filters( 'creativityarchitect_alphabetize_google_fonts', true ) ) {
			asort( $fonts );
		}

		// Filter to allow us to modify the fonts array
		return apply_filters( 'creativityarchitect_google_fonts_array', $fonts );
	}
}

if ( ! function_exists( 'creativityarchitect_get_all_google_fonts_ajax' ) ) {
	add_action( 'wp_ajax_creativityarchitect_get_all_google_fonts_ajax', 'creativityarchitect_get_all_google_fonts_ajax' );
	/**
	 * Return an array of all of our Google Fonts.
	 *
	 *
	 */
	function creativityarchitect_get_all_google_fonts_ajax() {
		// Bail if the nonce doesn't check out
		if ( ! isset( $_POST[ 'creativityarchitect_customize_nonce' ] ) || ! wp_verify_nonce( sanitize_key( $_POST[ 'creativityarchitect_customize_nonce' ] ), 'creativityarchitect_customize_nonce' ) ) {
			wp_die();
		}

		// Do another nonce check
		check_ajax_referer( 'creativityarchitect_customize_nonce', 'creativityarchitect_customize_nonce' );
		...
		$fonts = creativityarchitect_get_all_google_fonts();
...
if ( ! function_exists( 'creativityarchitect_get_google_font_variants' ) ) {
	/**
	 * Wrapper function to find variants for chosen Google Fonts
	 * Example: creativityarchitect_get_google_font_variation( 'Open Sans' )
	 *
	 */
	function creativityarchitect_get_google_font_variants( $font, $key = '' ) {
		// Don't need variants if we're using a system font
		if ( in_array( $font, creativityarchitect_typography_default_fonts() ) ) {
			return;
		}
		...
		if ( $variants ) {
			...
			return implode(',', apply_filters( 'creativityarchitect_typography_variants', $output ) );
		}
	}
}

if ( ! function_exists( 'creativityarchitect_get_google_font_category' ) ) {
	/**
	 * Wrapper function to find the category for chosen Google Font
	 * Example: creativityarchitect_get_google_font_category( 'Open Sans' )
	 *
	 *
	 *
	 ...
	 */
	function creativityarchitect_get_google_font_category( $font, $key = '' ) {
...
if ( ! function_exists( 'creativityarchitect_get_font_family_css' ) ) {
	/**
	 ...
	 */
	function creativityarchitect_get_font_family_css( $font, $settings, $default ) {
		$creativityarchitect_settings = wp_parse_args(
		...
		$no_quotes = array(
			...
			apply_filters( 'creativityarchitect_typography_system_stack', '-apple-system, system-ui, BlinkMacSystemFont, "Segoe UI", Helvetica, Arial, sans-serif, "Apple Color Emoji", "Segoe UI Emoji", "Segoe UI Symbol"' )
		);

		// Get our font
		$font_family = $creativityarchitect_settings[ $font ];

		if ( 'System Stack' == $font_family ) {
			$font_family = apply_filters( 'creativityarchitect_typography_system_stack', '-apple-system, system-ui, BlinkMacSystemFont, "Segoe UI", Helvetica, Arial, sans-serif, "Apple Color Emoji", "Segoe UI Emoji", "Segoe UI Symbol"' );
		}
		...
		if ( in_array( $font_family, $no_quotes ) ) {
			...
			$wrapper_start = '"';
			$wrapper_end = '"' . creativityarchitect_get_google_font_category( $font_family, $font );
		}
...
if ( ! function_exists( 'creativityarchitect_add_to_font_customizer_list' ) ) {
	add_filter( 'creativityarchitect_typography_customize_list', 'creativityarchitect_add_to_font_customizer_list' );
	/**
	 ...
	 *
	 */
	function creativityarchitect_add_to_font_customizer_list( $fonts ) {
		// Bail if we don't have our defaults
		if ( ! function_exists( 'creativityarchitect_get_default_fonts' ) ) {
			return;
		}
		...
		$all_fonts = creativityarchitect_get_all_google_fonts();
		$select_fonts = creativityarchitect_get_all_google_fonts( apply_filters( 'creativityarchitect_number_of_fonts', 200 ) );

		foreach ( $font_settings as $setting ) {
			// If we don't have a setting, keep going
			if ( ! isset( $creativityarchitect_settings[ $setting ] ) ) {
				continue;
			}

			$id = strtolower( str_replace( ' ', '_', $creativityarchitect_settings[ $setting ] ) );

			if ( array_key_exists( $id, $select_fonts ) || in_array( $creativityarchitect_settings[ $setting ], creativityarchitect_typography_default_fonts() ) ) {
				continue;
			}

			$fonts[ strtolower( str_replace( ' ', '_', $creativityarchitect_settings[ $setting ] ) ) ] = array(
				'name' => $creativityarchitect_settings[ $setting ],
		...
		if ( function_exists( 'creativityarchitect_secondary_nav_get_defaults' ) ) {
			$secondary_nav_settings = wp_parse_args(
				get_option( 'creativityarchitect_secondary_nav_settings', array() ),
				creativityarchitect_secondary_nav_get_defaults()
			);

			$secondary_nav_id = strtolower( str_replace( ' ', '_', $secondary_nav_settings[ 'font_secondary_navigation' ] ) );

			if ( ! array_key_exists( $secondary_nav_id, $select_fonts ) && ! in_array( $secondary_nav_settings[ 'font_secondary_navigation' ], creativityarchitect_typography_default_fonts() ) ) {
