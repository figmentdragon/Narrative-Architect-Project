...
This is the core THEMENAME file where most of the
...
function THEMENAME_rss_version() { return ''; }

// remove WP version from scripts
function THEMENAME_remove_wp_ver_css_js( $src ) {
	if ( strpos( $src, 'ver=' ) )
...
function THEMENAME_remove_wp_widget_recent_comments_style() {
...
function THEMENAME_post_comments_feed_link() {
    return;
}

// remove injected CSS from gallery
function THEMENAME_gallery_style($css) {
...
function THEMENAME_scripts() {
  ...
  if (!is_admin()) {
		...
		wp_register_script( 'THEMENAME-modernizr', get_stylesheet_directory_uri() . 'assets/scripts/js/libs/modernizr.custom.min.js', array(), '2.5.3', false );

		// register main stylesheet
		wp_register_style( 'THEMENAME-stylesheet', get_stylesheet_directory_uri() . 'style.css', array(), '', 'all' );

		// ie-only style sheet
		wp_register_style( 'THEMENAME-ie-only', get_stylesheet_directory_uri() . '/assets/scripts/css/ie.css', array(), '' );
    ...
    if ( is_singular() AND comments_open() AND (get_option('thread_comments') == 1)) {
		...
		wp_register_script( 'THEMENAME-js', get_stylesheet_directory_uri() . '/library/js/scripts.js', array( 'jquery' ), '', true );

		// enqueue styles and scripts
		wp_enqueue_script( 'THEMENAME-modernizr' );
		wp_enqueue_style( 'THEMENAME-stylesheet' );
		wp_enqueue_style( 'THEMENAME-ie-only' );

		$wp_styles->add_data( 'THEMENAME-ie-only', 'conditional', 'lt IE 9' ); // add conditional wrapper around ie stylesheet
...
function THEMENAME_theme_support() {
	...
	register_nav_menus(
		array(
			'primary'	                                  =>	__( 'Primary Menu', 'creativityarchitect' ), // Register the Primary menu
...
// Add support for experimental link color control.
	...
	if ( is_customize_preview() ) {
		...
		add_theme_support( 'starter-content', creativity_architect_get_starter_content() );
		}
} /* end THEMENAME theme support */
...
function THEMENAME_button_shortcode( $atts, $content = null ) {
...
function THEMENAME_filter_ptags_on_images($content){
...
function THEMENAME_excerpt_more($more) {
	...
	return '...  <a class="excerpt-read-more" href="'. get_permalink( $post->ID ) . '" title="'. __( 'Read ', 'THEMENAME' ) . esc_attr( get_the_title( $post->ID ) ).'">'. __( 'Read more &raquo;', 'THEMENAME' ) .'</a>';
}
